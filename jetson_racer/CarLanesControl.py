...
lane detection and jetson racer car control script
for autonomous driving in by lane following
...
def display_lines(frame, lines, line_color=(0, 0, 255), line_width=20):
    ...
    if (len(lines) == 2):
		...
		# get the the slope of each line
		...
		# get the average of both lines slope
        slope_avg = slope_R - slope_L
		
		# initialize and normalize the steering to be in range -1 to 1
	...
	# if 1 line detected in the frame
    elif (len(lines) == 1):
		...
		# increase the steering to get back to the center
        steering_up = steering * (3.5)

		# check if the line passed the center circle for extra steering to the center
        if slope < 0:
		...
		# pass the car steering control the new steering data
        car.steering = steering_up
		
        print("steering org  ", steering, " actual steering ", steering_up)
        cv.putText(frame, "steering: " + str(steering_up), (400, 200), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
...
if __name__ == '__main__':
	...
	# intialize the car road camera
    cap = cv.VideoCapture(1, cv.CAP_V4L)

    print('Started')
	
	# intialize the car control system
    car = NvidiaRacecar()

	# intialize the car speed
    car.throttle = 0.49
    ...
    while cap.isOpened():
		...
		# orange color mask for the lane lines 
        lowerWhite = np.array([4, 45, 45])
        upperWhite = np.array([15, 255, 255])
		
		# treshold frame
        thres_1 = cv.inRange(hsv_image, lowerWhite, upperWhite)
        ...
        # cv.putText(line_image, "Detecting Lane", (50, 50), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
